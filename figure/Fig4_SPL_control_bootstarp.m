...
if cal
    ...
    for ss=1:92
        ...
        for nn=1:size(IT,2)
            ...
            phase_fef =squeeze(phase_fef_(:,:,t1_ld<t_h&t_h<t2_ld));
            phase_it =squeeze(phase_it_(:,:,t1_ld<t_h&t_h<t2_ld));
            diff_phi=squeeze(diff_phi_(:,:,t1_ld<t_h&t_h<t2_ld));
            for condi=1:1
                ...
                ix_h_cr_T=ix_h_cr;ix_h_wr_T=ix_h_wr;
                
                phase_fef_cr = []; phase_fef_cr=squeeze(phase_fef(ix_h_cr,:,:));
                phase_it_cr = [];phase_it_cr=squeeze(phase_it(ix_h_cr,:,:));
                diff_phi_cr = [];diff_phi_cr=squeeze(diff_phi(ix_h_cr,:,:));
                rate_it_cr = [];rate_it_cr = rate_it(ix_h_cr,:);
                
                phase_fef_wr = []; phase_fef_wr=squeeze(phase_fef(ix_h_wr,:,:));
                phase_it_wr = [];phase_it_wr=squeeze(phase_it(ix_h_wr,:,:));
                diff_phi_wr = [];diff_phi_wr=squeeze(diff_phi(ix_h_wr,:,:));
                rate_it_wr = [];rate_it_wr = rate_it(ix_h_wr,:);
                
                
                for bts=1:N_bts
                    ix_h=[];ix_h=randi(size(phase_fef_cr,1),size(phase_fef_cr,1),1);
                    phase_fef_cr_bts = []; phase_fef_cr_bts=squeeze(phase_fef_cr(ix_h,:,:));
                    phase_it_cr_bts = [];phase_it_cr_bts=squeeze(phase_it_cr(ix_h,:,:));
                    diff_phi_cr_bts = [];diff_phi_cr_bts=squeeze(diff_phi_cr(ix_h,:,:));
                    rate_it_cr_bts = [];rate_it_cr_bts = rate_it_cr(ix_h,:);
                    
                    if N_wr>0
                        
                        ix_h=[];ix_h=randi(size(phase_fef_wr,1),size(phase_fef_wr,1),1);
                        phase_fef_wr_bts = []; phase_fef_wr_bts=squeeze(phase_fef_wr(ix_h,:,:));
                        phase_it_wr_bts = [];phase_it_wr_bts=squeeze(phase_it_wr(ix_h,:,:));
                        diff_phi_wr_bts = [];diff_phi_wr_bts=squeeze(diff_phi_wr(ix_h,:,:));
                        rate_it_wr_bts = [];rate_it_wr_bts = rate_it_wr(ix_h,:);
                    end
                    % ix_h_cr=
                    for fi=1:length(freqs2use)
                        
                        val_fef=[];val_it=[]; val_shuff=[];val_shuff=[];
                        [val_fef,val_it,val_shuff]=spl_per_condition_bahmani(phase_fef_cr_bts,phase_it_cr_bts,rate_it_cr_bts,N_sam_sp);
                        
                        
                        
                        val_fef_it_cr(nn,condi,fi,bts)=(val_fef);
                        val_it_it_cr(nn,condi,fi,bts)=(val_it);
                        val_shuff_it_cr(nn,condi,fi,bts)=(val_shuff);
                        ...
                        if N_wr>0
                            ...
                            val_fef_it_wr(nn,condi,fi,bts)=(val_fef);
                            val_it_it_wr(nn,condi,fi,bts)=(val_it);
                            val_shuff_it_wr(nn,condi,fi,bts)=(val_shuff);
                        else
                            val_fef_it_wr(nn,condi,fi,bts)=nan;
                            val_it_it_wr(nn,condi,fi,bts)=nan;
                            val_shuff_it_wr(nn,condi,fi,bts)=nan;
                        end
        ...
        Info(ss).val_fef_it_cr=val_fef_it_cr;
        Info(ss).val_it_it_cr=val_it_it_cr;
        Info(ss).val_shuff_it_cr=val_shuff_it_cr;
        
        Info(ss).val_fef_it_wr=val_fef_it_wr;
        Info(ss).val_it_it_wr=val_it_it_wr;
        Info(ss).val_shuff_it_wr=val_shuff_it_wr;
...
%         end
%         
%         Info(ss).val_fef_fef_cr=val_fef_fef_cr;
%         Info(ss).val_it_fef_cr=val_it_fef_cr;
%         Info(ss).val_shuff_fef_cr=val_shuff_fef_cr;
%         
%         Info(ss).val_fef_fef_wr=val_fef_fef_wr;
%         Info(ss).val_it_fef_wr=val_it_fef_wr;
%         Info(ss).val_shuff_fef_wr=val_shuff_fef_wr;
%         
        ss
        toc
    end
    ...
    save('D:\Data\Reaserch\Thesis\Data Analysis\Mat\SPL\spl_ppc_per_session_bahmani_bts.mat'...
        , 'Info','freqs2use')
end
...
ind_ses_itphase=session_selection(4);
...
%%
ind_ses_fefphase=session_selection(2);   %% FOr preferance put it 7
