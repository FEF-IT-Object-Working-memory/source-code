function [conditions,performance]=session_stats_PPL()
 ...
 load('F:\Data\Reaserch\Thesis\Data Analysis Reconfiguration\Data\gen_session_inf_PPL.mat')
...
for ss =1:92
    ...
    for condi =1:6
        ...
        ix_h = [];
        ix_h = ismember(gen_session_inf(ss).condition{:},obj_h)&...
            (gen_session_inf(ss).bhv{:}==1)&(~gen_session_inf(ss).FEF_artifact)...
            &(~gen_session_inf(ss).IT_artifact);     
        conditions.cr(ss,condi) = sum(ix_h);
        
        ix_h = [];               
        ix_h = ismember(gen_session_inf(ss).condition{:},obj_h)&...
            (gen_session_inf(ss).bhv{:}==0)&(~gen_session_inf(ss).FEF_artifact)...
            &(~gen_session_inf(ss).IT_artifact);        
